...
def _run_evaluation(
    ...
    try:
        ...
        for group_key, (X_test_group, y_test_group) in test_data_dict.items():
            ...
            # 强制将两者都转换为 1D 数组 (N,) 以防止 (N,1)-(N,) 广播错误
            true_loss_group = np.asarray(y_test_group, dtype=float).ravel()
            pred_fixed_group = np.asarray(pred_fixed_group, dtype=float).ravel()
            ...
            if calibration_mode == 'multiplicative':
                ...
                # 现在 residuals 保证是 (N,)，'effect' 保证是标量
